...
def download_image(_system_object_id, image_url, xml_filename, writer):
    if not image_url.lower().endswith(ACCEPTED_IMAGE_EXTS):
        logger.warning(_system_object_id, image_url, "is not a valid image")
        return

    if image_url == metadata.getLatestImageDownloadUrlForFile(xml_filename):
        logger.info("Image already downloaded", _system_object_id, image_url)
        return
    ...
    try:
        ...
        if retries >= MAX_RETRIES:
            logger.warning("Could not download", _system_object_id, image_url)
            return
        ...
        logger.error(f"Error processing image for {_system_object_id}: {e}")
...
def download_pdf(_system_object_id, pdf_url, xml_filename, writer):
    ...
    if pdf_url == metadata.getLatestPdfDownloadUrlForFile(xml_filename):
        logger.info("PDF already downloaded", _system_object_id)
        return
    ...
    try:
        ...
        if retries >= MAX_RETRIES:
            logger.warning(f"Could not download PDF for {_system_object_id}: {pdf_url}")
            return
        ...
        logger.error(f"Error downloading PDF for {_system_object_id}: {e}")
